...
class TestClaudeSession:
    ...
    def test_claude_session_initialization(self, tmp_path):
        ...
        session = Session('abc123-def456-ghi789', session_path, tool='claude',
    ...
    def test_claude_session_name(self, tmp_path):
        ...
        session = Session('test-session-id', session_path, tool='claude',
                          history_data=history_data)
    ...
    def test_claude_session_name_truncation(self, tmp_path):
        ...
        session = Session('test-session', session_path, tool='claude',
                          history_data=history_data)
    ...
    def test_claude_session_size_empty_directory(self, tmp_path):
        ...
        session = Session('empty-session', session_path, tool='claude',
                          history_data=history_data)
    ...
    def test_discover_claude_sessions(self, tmp_path, monkeypatch):
        ...
        assert [s.session_id for s in sessions] == ['session-003', 'session-002', 'session-001']
...
class TestMultiSessionExport:
    ...
    def test_export_multiple_codex_sessions(self, tmp_path):
        ...
        for i in range(2):
            ...
            (session_path / f'rollout-{i}.jsonl').write_text(json.dumps({'test': f'data {i}'}))
            (session_path / 'metadata.json').write_text(json.dumps({'meta': f'value {i}'}))
        ...
        with tarfile.open(archive_path, 'r:gz') as tar:
            ...
            assert any(f'codex-session-0' in m for m in members)
            assert any(f'codex-session-1' in m for m in members)
            assert 'metadata.json' in members
    ...
    def test_export_multiple_claude_sessions(self, tmp_path, monkeypatch):
        ...
        for i in range(3):
            ...
            sessions.append(Session(sid, session_path, tool='claude',
                                   history_data=entry))
...
class TestClaudeHistoryFileReading:
    ...
    def test_claude_history_file_reading(self, tmp_path, monkeypatch):
        ...
        history_file = tmp_path / 'history.jsonl'
        history_entries = [
            json.dumps({'sessionId': 'other-session', 'display': 'Other', 'timestamp': 1000}),
            json.dumps({'sessionId': 'test-session-history', 'display': 'History Test', 'timestamp': 2000}),
            json.dumps({'sessionId': 'another-session', 'display': 'Another', 'timestamp': 3000})
        ]
        history_file.write_text('\n'.join(history_entries))
