...
class TestExportScript:
    ...
    def test_display_session_menu(self, mock_stdout, tmp_path):
        ...
        # Mock user input to select first session
    ...
    def test_display_session_menu_invalid_selection(self, mock_input, mock_stdout, tmp_path):
        """Test session menu with invalid selection then valid."""
        # Create test session
        session_id = 'sess-test001'
        session_path = tmp_path / session_id
        session_path.mkdir()
        (session_path / f'{session_id}.json').write_text(json.dumps({'title': 'Test Session'}))
    ...
    def test_display_session_menu_multiple_selection(self, mock_stdout, tmp_path):
        ...
        # Mock user input to select sessions 1, 3, 5
    ...
    def test_display_session_menu_range_selection(self, mock_stdout, tmp_path):
        ...
        # Mock user input to select range 1-3
...
class TestCLIArguments:
    ...
    def test_parse_arguments_output_dir(self):
        ...
        with patch.object(sys, 'argv', ['export_session.py', '--output-dir', test_path]):
            args = parse_arguments()
